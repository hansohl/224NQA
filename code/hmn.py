...
def HMN(U, h_i, s_prev, e_prev, scope_name):
    with tf.variable_scope(scope_name) as scope:
        ...
        m1 = tf.reduce_max(tf.matmul(Ur, W1) + b1, axis=0) #m1 is batch_size x question_length x hidden_size x pool_size
        m2 = tf.reduce_max(tf.matmul(m1, W2) + b2, axis=0) #m2 is batch_size x question_length x hidden_size
